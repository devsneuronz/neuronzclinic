...
type RawContactNote = Record<string, unknown>
...
async function supabaseRequest(path: string, init?: RequestInit) {
  if (!SUPABASE_REST_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error("Missing Supabase REST configuration for contact notes. Add SUPABASE_SERVICE_ROLE_KEY to .env.local and restart the dev server.")
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const select = ["id", "chat_id", "contact_phone", "content", "created_at", "updated_at"].join(",")
    const response = await supabaseRequest(`contact_notes?select=${select}&chat_id=eq.${encodeURIComponent(chatId)}&order=created_at.desc`)

    if (!response.ok) {
      return NextResponse.json({ message: await response.text() }, { status: response.status })
    }

    return NextResponse.json({ notes: (await response.json()) as RawContactNote[] })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json().catch(() => null)) as RawContactNote | null
    ...
    const note = {
      chat_id: chatId,
      contact_phone: getNullableString(body.contact_phone),
      content,
    }
    ...
    const data = (await response.json()) as RawContactNote[]
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const response = await supabaseRequest(`contact_notes?id=eq.${encodeURIComponent(id)}`, {
      method: "DELETE",
      headers: {
