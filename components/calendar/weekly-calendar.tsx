      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border p-4 md:px-6 min-h-15.25">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 justify-center min-w-0">
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-base sm:text-lg font-semibold capitalize text-foreground truncate max-w-[180px] sm:max-w-none px-1">{getHeaderTitle(currentDate, activeView)}</h1>

            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={goToNext} aria-label="Proximo periodo">
        ...
        <div className="grid grid-cols-2 gap-2 md:flex md:items-center md:gap-3 w-full md:w-auto">
          <Button className="gap-2 bg-theme-primary text-white hover:bg-theme-primary/90 text-xs sm:text-sm px-3 sm:px-4" onClick={() => openAppointmentDialog(new Date())}>
            ...
            {activeView === "Mês" ? (
                  ...
                  {filteredAppointments.length === 0 ? (
                    ...
                    <div className="divide-y divide-border">
                      {filteredAppointments.map((appointment) => {
                        ...
                        const professional = professionalLabels.get(appointment.professionalId) || appointment.professional;
                        return (
                          <div key={appointment.id} className="flex flex-col sm:grid sm:grid-cols-[120px_1fr_1fr_1fr_100px] gap-2 sm:gap-4 px-4 py-3 hover:bg-muted/20 transition-colors">
              ...
              <div>
                ...
                <div className={cn("relative flex min-h-[960px]", activeView == "Semana" && "min-w-341.5")}>
                  <div className="w-16 flex-shrink-0 border-r border-border sticky left-0 bg-card/85 backdrop-blur-xs z-40 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                    {timeSlots.map((hour) => (
                  ...
                  {visibleDays.map((day) => {
                    ...
                    return (
                      <div
                        key={day.toISOString()}
                        className={cn("relative flex-1 cursor-crosshair border-r border-border select-none last:border-r-0", isSameDay(day, new Date()) && "bg-primary/5")}
