...
function getStatusColor(status: string) {
  ...
  if (normalizedStatus.includes("cancel")) return "border-destructive bg-destructive/10 text-destructive"
  if (normalizedStatus.includes("pend")) return "border-amber-500 bg-amber-500/10 text-amber-700 dark:text-amber-300"
  if (normalizedStatus.includes("confirm")) return "border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
...
export function WeeklyCalendar() {
  ...
  const selectedAppointmentPatient = useMemo(
    () => options.patients.find((patient) => patient.id === appointmentPatientId),
    [appointmentPatientId, options.patients],
  )
  const patientSearchResults = useMemo(() => {
    ...
    return options.patients
      .filter((patient) => normalizeText(patient.label).includes(query))
      .slice(0, 8)
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return appointments.filter((appointment) => {
      ...
      const searchablePatient = normalizeText([
        appointment.patient,
        appointment.phone,
  ...
  function renderAppointmentCard(appointment: CalendarAppointment, compact = false) {
    const startDate = getAppointmentDate(appointment)
    ...
    return (
      <button
        ...
        type="button"
        className={cn(
          "group h-full w-full overflow-hidden rounded-md border border-border/70 bg-card text-left shadow-sm transition-all hover:-translate-y-px hover:border-primary/50 hover:shadow-md",
          compact ? "p-1.5" : "p-2",
        )}
      ...
      >
        <div className="flex h-full gap-2">
          <div className={cn("w-1 shrink-0 rounded-full", getStatusColor(appointment.status).split(" ")[0].replace("border-", "bg-"))} />
          <div className="min-w-0 flex-1">
            ...
            {!compact && appointment.phone && <p className="truncate text-[11px] text-muted-foreground">{appointment.phone}</p>}
            <div className="mt-1 flex min-w-0 items-center justify-between gap-2 border-t border-border/50 pt-1">
              <p className="truncate text-[11px] font-semibold text-primary">{appointment.type || "Sem tipo"}</p>
              <span className={cn("shrink-0 rounded-sm px-1.5 py-0.5 text-[10px] font-semibold", getStatusColor(appointment.status))}>
                {appointment.status}
              </span>
            </div>
          </div>
  ...
  return (
    <div className="flex h-screen flex-1 flex-col bg-background">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          <Button className="gap-2 bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => openAppointmentDialog(new Date())}>
          ...
          {views.map((view) => (
            <button
              ...
              className={cn(
                ...
                activeView === view ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground",
              )}
            >
              {view}
            </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-auto">
          ...
          {activeView === "Mês" ? (
            <div className="grid min-h-full grid-cols-7 border-b border-border">
              {visibleDays.map((day) => {
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={cn("min-h-32 cursor-pointer border-r border-t border-border p-2 last:border-r-0 hover:bg-muted/40", !isSameMonth(day, currentDate) && "bg-muted/30 text-muted-foreground")}
                    onClick={() => {
                      setCurrentDate(day)
                      setActiveView("Dia")
                    }}
                  >
                    <div className="mb-2 flex items-center justify-between">
                      <span className={cn("text-xs font-semibold uppercase", isSameDay(day, new Date()) && "text-primary")}>{format(day, "EEE", { locale: ptBR })}</span>
                      <span className={cn("flex h-6 w-6 items-center justify-center rounded-full text-sm", isSameDay(day, new Date()) && "bg-primary text-primary-foreground")}>{format(day, "d")}</span>
                    </div>
                    <div className="space-y-1.5">
                      {dayAppointments.slice(0, 3).map((appointment) => renderAppointmentCard(appointment, true))}
                  ...
                  filteredAppointments.map((appointment) => {
                    ...
                    return (
                      <div key={appointment.id} className="grid grid-cols-[120px_1fr_1fr_1fr_120px] gap-4 border-b border-border px-4 py-3 last:border-b-0">
                        ...
                        <span className={cn("truncate text-sm font-medium", getStatusColor(appointment.status).split(" ").at(-1))}>{appointment.status}</span>
                      </div>
                    )
                  })
                )}
            ...
            <div>
              <div className="sticky top-0 z-10 flex border-b border-border bg-card">
                ...
                {visibleDays.map((day) => (
                  <div key={day.toISOString()} className={cn("flex-1 border-r border-border px-2 py-3 text-center last:border-r-0", isSameDay(day, new Date()) && "bg-primary/5")}>
                    <p className={cn("text-xs font-semibold uppercase", isSameDay(day, new Date()) ? "text-primary" : "text-muted-foreground")}>{format(day, "EEE", { locale: ptBR })}</p>
                    <p className={cn("mt-0.5 text-sm", isSameDay(day, new Date()) ? "font-bold text-primary" : "font-medium text-foreground")}>{format(day, "dd/MM")}</p>
                  </div>
                ))}
              </div>

              <div className="relative flex min-h-[960px]">
                <div className="w-16 flex-shrink-0 border-r border-border">
                  {timeSlots.map((hour) => (
                    <div key={hour} className="h-[60px] border-b border-border pr-2 text-right">
                      <span className="text-xs font-bold text-muted-foreground">{String(hour).padStart(2, "0")}</span>
                    </div>
                  ))}
                </div>
                ...
                {visibleDays.map((day) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {daySelection && (
                        <div
                          className="pointer-events-none absolute left-1 right-1 z-10 rounded-md border border-dashed border-primary bg-primary/15"
        ...
        <DialogContent className="max-w-lg">
          {selectedAppointment && (
            <>
              ...
              <div className="space-y-4">
                <div className={cn("rounded-md border-l-4 bg-card p-4", getStatusColor(selectedAppointment.status))}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-foreground">
                        {professionalLabels.get(selectedAppointment.professionalId) || selectedAppointment.professional}
                      </p>
                      <p className="mt-1 truncate text-sm text-muted-foreground">{selectedAppointment.patient}</p>
                    </div>
                    <span className="rounded-md bg-background/80 px-2 py-1 text-xs font-bold text-foreground">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span>
                      {getAppointmentDate(selectedAppointment)
                        ? format(getAppointmentDate(selectedAppointment) as Date, "dd/MM/yyyy", { locale: ptBR })
                        : "--"}{" "}
                      · {formatAppointmentTime(selectedAppointment)}
                    </span>
                  </div>
      ...
      <Dialog open={isAppointmentDialogOpen} onOpenChange={setIsAppointmentDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {appointmentStartDateTime ? format(new Date(appointmentStartDateTime), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR }) : "Selecione os dados do agendamento."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form className="space-y-4" onSubmit={handleCreateAppointment}>
            ...
            <div className="space-y-2">
              ...
              {selectedAppointmentPatient && (
                <p className="text-xs text-muted-foreground">Selecionado: {selectedAppointmentPatient.label}</p>
              )}
            </div>
