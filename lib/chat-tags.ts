...
const LEGACY_ANUNCIO_INTEREST_ID = "ca73cc42-dd00-4ffe-9069-8ad684847f2b";
...
function getTagFromObject(value: TagLike): ChatTag | null {
  ...
  const label = asString(source.Tag) || asString(source.tag) || asString(source.label) || asString(source.name) || asString(source.Nome) || rawId;

  if (!label && !rawId) return null;

  const id = rawId || label;
  const color = normalizeColor(source.HEXCOR) || normalizeColor(source.hexcor) || normalizeColor(source.hex_status) || normalizeColor(source.color) || normalizeColor(source.cor);
...
export function resolveChatTags(tags: ChatTag[], catalog: ChatTag[]): ChatTag[] {
  ...
  for (const tag of catalog) {
    ...
    if (normalizeLookup(tag.label) === "anuncio") {
      catalogByKey.set(LEGACY_ANUNCIO_INTEREST_ID, tag);
    }
  }

  return tags.map((tag) => {
    const resolved = catalogByKey.get(tag.id.trim().toLowerCase()) ?? catalogByKey.get(tag.label.trim().toLowerCase());
